...
export class CameraService {
  ...
  constructor(
    @InjectModel('Camera') private readonly cameraModel: Model<Camera>,
          ...
          try {
            ...
            child.stdout.on('data', async data => {
              ...
              if (output.split('.').pop() === `jpg`) {
                setTimeout(() => {
                  fs.readFile(`${process.env.ASSETS_PATH}/${output}`, function(
                    err,
                    data,
                  ) {
                    if (err) {
                      ...
                      s3.putObject(params, async function(err, data) {
                        if (err) {
                          ...
                          setTimeout(() => {
                            fs.unlinkSync(
                              `${process.env.ASSETS_PATH}/${output}`,
                            );
                          }, 2000);
                        }
                      });
  ...
  async updateOne(
    ...
    try {
      const result = await this.cameraModel.updateOne(
        { _id: id },
        { name, username, password, ip, port, rtspUrl, backupMode },
      );
      return true;
    } catch (error) {
      return false;
  ...
  async recordStreamPerTime(
    ...
    if (count === 0) {
      try {
        await execSync(
          `mkdir -p -v ${process.env.ASSETS_PATH}/${camID}/record`,
        );
      } catch (error) {}
    }
    console.log('a', time);
    ...
    setTimeout(async () => {
      const watcher = chokidar.watch(
        `${process.env.ASSETS_PATH}/${camID}/record`,
        {
          ignored: ['.DS_Store'],
          persistent: true,
      ...
      const ffmpeg = spawn('ffmpeg', [
        ...
        `${
          process.env.ASSETS_PATH
        }/${camID}/record/${nowTime.toString()}_%03d.mp4`,
      ]);
      ...
      ffmpeg.on('exit', async code => {
        ...
        const recordModeTask = await this.taskService.findTask(
          '0',
          userID,
          camID,
        );
        const recordedTask = await this.taskService.findTask(
          '3',
          userID,
          camID,
        );
        ...
        try {
          setTimeout(() => {
            ...
            if (files.length !== 0) {
              ...
              fs.readFile(
                `${process.env.ASSETS_PATH}/${camID}/record/${files[0]}`,
                function(err, data) {
                  if (err) {
                    console.log('fs error', err);
                  } else {
                    ...
                    s3.putObject(params, async function(err, data) {
                      if (err) {
                        ...
                        let duration;
                        exec(
                          `ffprobe -v error -show_entries format=duration -of default=noprint_wrappers=1:nokey=1 ${cdnUrl}`,
                          async (error, stdout, stderr) => {
                            ...
                            execSync(
                              `rmdir ${process.env.ASSETS_PATH}/${camID}/record`,
                            );
                          },
                        );
                      }
  ...
  async recordDetection(
    ...
    try {
      ...
      const recordModeTask = await this.taskService.findTaskWithoutUser(
        '0',
        _id,
      );
      if (recordModeTask) {
      ...
      child.stdout.on('data', async data => {
        ...
        if (timeStart !== '' && timeEnd !== '') {
          await this.camMotionService.addOne(
            userID,
            rtspUrl,
            null,
            timeStart,
  ...
  async motionDetection(_id: string, userID: string): Promise<any> {
    ...
    try {
      ...
      if (count === 0) {
        try {
          await execSync(
            `mkdir -p -v ${process.env.ASSETS_PATH}/${_id}/detect`,
          );
        } catch (error) {}
      }
      const motionDetectTask = await this.taskService.findTaskWithoutUser(
        '1',
        _id,
      );
      ...
      child.on('exit', async code => {
        ...
        try {
          setTimeout(() => {
            ...
            if (files.length !== 0) {
              ...
              const now = d.getDate() + '_' + month + '_' + d.getFullYear();
              fs.readFile(
                `${process.env.ASSETS_PATH}/${_id}/detect/${files[0]}`,
                function(err, data) {
                  if (err) {
                    console.log('fs error', err);
                  } else {
                      ...
                      if (err) {
                        ...
                        exec(
                          ...
                          async (error, stdout, stderr) => {
                            ...
                            execSync(
                              `rmdir ${process.env.ASSETS_PATH}/${_id}/detect`,
                            );
                          },
                        );
                      }
        ...
        const motionModeTask = await this.taskService.findTask(
          '1',
          userID,
          _id,
        );
  ...
  async uploadVideo(userID: string, cameraID: string, filePath: string) {
    fs.readFile(
      `${process.env.ASSETS_PATH}/${cameraID}/detect/${filePath}`,
      function(err, data) {
        if (err) {
          console.log('fs error', err);
        } else {
  ...
  testput() {
    this.recordStreamPerTime(
      '5ed3e22848d6943ed70ec47f',
      'rtsp://192.168.1.104:7777/h264_ulaw.sdp',
      '5e9471d6cbeb62504f03bc0b',
      10,
    );
    return true;
  }
              ...
              if (err) {
                ...
                s3.putObject(params, function(err, data) {
                  if (err) {
                    ...
                    resolve(
                      `https://clientapp.sgp1.digitaloceanspaces.com/${rtspUrl}/${output}`,
                    );
                  }
                });
              }
